...
const string vertex_file = "vertex_order_50000.txt";
//const string LBT_file = bound_dir + "lower_dist_matrix_10000.txt";
const string LBT_file = "lower_dist_matrix_50000.txt";
...
unordered_map<int,bool> t_closed[window_size+1];
...
void Bidirection_search(int src, int dest, int d_time, int LBT, int len, ofstream& out);
void Get_vertex(string file);
void Read_bound_file(string file, char type);
void readcase(string dir, bool, ofstream& out);
...
public:
	...
	int SP_TAG_search(int src, int dest, int time, int len, ofstream& out);	
...
int main()
{
	...
	int lamda[] = {-1,0,10,20,30,40};
	bool forward = true;
	for(auto lamb : lamda){
		if(lamb == -1){
			Out_dir = "./SP_Exectime_10/";
			...
			ofstream out;
			out.open(Out_dir+"sp_tag_result.txt");
			if(!out.is_open()){
				printf("Error open file: sp_tag_result.txt\n");			
			}
			readcase("./", forward, out);
			forward = false;
			out.close();		
		}
		else{
			Out_dir = "./Exectime_10_1_impromptus_" + to_string(lamb) +"/";	
			...
			theta = lamb;
			ofstream out;
			out.open(Out_dir+"1st_impt10_"+to_string(lamb)+".txt");
			if(!out.is_open()){
				printf("Error open file: 1st_impt10_%d.txt\n", lamb);	
			}
			readcase("./", forward,out);
			out.close();		
		}		
...
int Object::SP_TAG_search(int src, int dest, int time, int len, ofstream& out){
	...
	while(!Q.empty()){
		...
		if(u == dest){
			...
			out << to_string(time) << " " << to_string(len) << " " << to_string(w) << " " << to_string(closed.size()) << "\n";
			return w;
		}
		...
		for(auto &v : tag_graph[u]){
			if(dist[0][v.first] > v.second[w]){
				dist[0][v.first] = v.second[w];
				parent[0][v.first] = u;
				closed[v.first] = true;
				Q.push({dist[0][v.first], v.first});
			}
		}
...
void readcase(string dir_path, bool type, ofstream& searchfile){
	...
	while(ep = readdir(dp)){
		...
		if(regex_match(file, regex(patt)) ){
			...
			while(infile.good()){
				...
				for(int k=0; k<3 && len>100; k++){
					int d_time = dep_times[k];
					//string out = Out_dir + "sp_" + file;
					//string out = Out_dir + "par-sp_" + file;
					string out;
					if(type)
						out = Out_dir + "sp-tag_" + to_string(d_time) + "_" + file;
					else
						out = Out_dir + "bi-dict_" + to_string(d_time) + "_" + file;
					...
					if(!outfile.is_open()){
						...
						//src, dest, time, len);
						if(type)
							obj.SP_TAG_search(src,dest,d_time,len,searchfile);
						else
							Bidirection_search(src, dest, d_time, LBT, len,searchfile);
						auto t2 = steady_clock::now();
...
void Bidirection_search(int src, int dest, int d_time, int LBT, int len, ofstream& out){
	...
	/*
	printf("src: %d depature at %d arrival dest: %d at %d  (min_ford: %d,  min_trace: %d LBT: %d, len: %d) f_close: %ld t_closed: %ld\n\n", 
							src, d_time, dest, min(min_trace, min_ford), min_ford, min_trace, LBT, len, t_closed[0].size(), t_closed[1].size());*/
	
	out << "src: " << to_string(src) << "depature at " << to_string(d_time) << " arrival dest: " << to_string(dest) << " at " << to_string(min(min_trace, min_ford)) << " (min_ford: " << to_string(min_ford) << ", min_trace: " << to_string(min_trace) << " LBT: " << to_string(LBT) << " len: " << to_string(len) << ") f_close: " <<to_string(t_closed[0].size()) <<" t_close: " << to_string(t_closed[1].size()) << "\n";
